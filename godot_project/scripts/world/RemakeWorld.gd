...
## its front faces its street (RingCoords convention), at the HIGHEST ground under its massing
...
##
## build(root, radius, segments, settlements) -- a coroutine (a building per frame); settlements: names to place (the inventory's),
...
static func build(root: Node3D, radius: float, segments: int, settlements: Array) -> Dictionary:
	...
	for e in pl:
		...
		for lx in [e.fmin[0], (e.fmin[0] + e.fmax[0]) * 0.5, e.fmax[0]]:
			for lz in [e.fmin[1], (e.fmin[1] + e.fmax[1]) * 0.5, e.fmax[1]]:
				...
				h = maxf(h, TerrainHeight.elevation_at(radius, segments, s + ds, x + dx))
		var pos := _flat(radius, segments, s, x) + up * h
	...
	var info: Dictionary = await RemakeLodClusters.build(root, entries)
