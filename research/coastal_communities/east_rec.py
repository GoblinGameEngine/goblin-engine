#!/usr/bin/env python3
"""Write the deck's coastal catalog records (PCR / BRW and the great bridges XBR / XRR) from a spec file.
(Adapted from west_rec.py: the coast comes from the inventory, bridges are included, claims go to
_claims_coastal_east.txt and are checked against every _claims_coastal_*.txt.)

  east_rec.py near "File:X.jpg" [R]     other geotagged Commons files within R m (default 40)
  east_rec.py cat "Category:X"          files in a Commons category (a building's own category)
  east_rec.py make spec.json            fetch each record's photos, write remake/catalog/<ID>.json,
                                        claim its source ids in remake/catalog/_claims_coastal_east.txt
...
A claim already in any coastal claims file is refused."""
import glob
import json
import os
import sys
...
CLAIMS = os.path.join(CAT, "_claims_coastal_east.txt")
...
def claimed():
    s = set()
    for p in glob.glob(os.path.join(CAT, "_claims_coastal*.txt")):
        if os.path.exists(p):
...
def make(spec_path):
    ...
    inv.update({b["id"]: b for b in json.load(open(INV))["bridges"]})
    taken = claimed()
    photo_owner = {}
    ...
    for r in json.load(open(spec_path)):
        ...
        rec = {"id": sid, "settlement": s.get("settlement"), "kind": s.get("kind", s.get("type")), "coast": s.get("coast", "east")}
        if "w" in s:
            rec["lot"] = {"w": s["w"], "d": s["d"]}
        elif sid.startswith(("XBR", "XRR")):
            rec["crossing"] = {k: s[k] for k in s if k not in ("id", "settlement", "kind", "coast")}
        else:
